...
import ThemeToggle from '@/admin/components/blocks/theme-toggle'
// import { MainNav } from "./main-nav";
//import { adminConfig } from "@/config/site";
import Dropdown from "@/admin/components/blocks/dropdown";
...
// import "./navbar.scss"
import { Person, Power } from '@/components/svg';
import { ToastContextType, useToastDispatch } from '@/components/toast-provider';
...
export default function Navbar(props) {
  ...
  const userDropdown = [
    { url: '/user/profile', icon: <Person />, title: 'Profile' },
    { url: '', icon: <Power />, title: 'Logout', onClick: Logout },
  ];

  return (   
    <header id="header" className="navbar">      
      <a href="/" className="logo">
        <Code className='icon'  fill="none" stroke="currentColor"  viewBox="0 0 24 24" />        
        <span className='ml-2'>{siteInfo.name}</span>
      </a>        
      <nav id="main-nav" className="flex flex-row flex-wrap items-right items-center justify-end gap-2">
        <ThemeToggle className="text-gray-600" />
        {siteInfo.mainNav?.length ? (         
          <ul id="menu-items" className='flex gap-2'>   
        ...
        <Dropdown label="Profile" items={userDropdown} className="dropdown-end " />
        {/*<DropdownMenu>
          <DropdownMenuTrigger>
