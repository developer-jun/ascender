...
const TagForm = () => {
  ...
  const { formFieldsInfo, formObject } = useTagForm();
  ...
  const handleSubmitResult = (values) => {  
    ...
    postRequest(values); // for results, looked into the useEffect
    setTimeout(() => {     
      reset(); 
      pageDispatch({ 
        type: 'set_field', 
      ...
      <div data-theme="nord" id="category-form" className="relative flex flex-col place-items-center custom-form p-5 rounded-md bg-gray-300 shadow-xl shadow-slate-950">
        <div className="mt-3 p-3 w-full items-center text-center">
          ...
          <Form onSuccessCallback={handleSubmitResult}           
            header={
              <Form.Header>
                <FormTitle className="font-bold text-xl text-center">Tag Form</FormTitle>
            ...
            footer={
              <Form.Footer className="text-left mt-2 inline-flex gap-1">
                <Button2 
                  className={`${!selectedItem?.tag_id ? 'hidden' : 'custom-btn btn-sm bg-cyan-950 text-white hover:bg-cyan-900'}`} 
                  type="reset" 
                ...
                <Button2 
                  ...
                  type="submit">{loading?<SVG icon='spinner' className='animate-spin mr-2 w-4 h-4' />:''}{selectedItem?.tag_id ? 'Update' : 'Create'}</Button2>
              </Form.Footer>
            }
          />          
        </div>
