...
export async function create(categoryData: Category) {
  try{
    ...
    let message: string = 'An error occured while creating the category. Please try again later in case a maintainance is ongoing. If issue persists, please contact support <admin@support.com>.';
    console.log('QUERY ERROR')
    console.log(error)
    return message;
  }
